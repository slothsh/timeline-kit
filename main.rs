...
struct Args {
    #[arg(value_delimiter = ' ', value_parser, default_value = ".", index = 1)]
    paths: Vec<String>,

    #[arg(long, short, default_value = ".")]
    out: String,

    #[arg(long, short, default_value = "utf-8")]
    encoding: String,
}
...
impl<'a> EDLParser<'a> {
    fn parse(input_path: &'a str, encoding: &'static encoding_rs::Encoding) -> Result<EDLSession, String> {
        ...
        while let Some(line_result) = all_lines.next() {
            match line_result {
                Ok(line) => {
                    ...
                    // println!("{}:{:?} {}", edl_parser.file_position, current_section, line);
    ...
    fn check_section(&mut self, line: &str) -> Option<(EDLSection, bool, bool)> {
        ...
        if line == "" {
            return None;
            // return Some((Ignore, false, false));
        }
...
fn main() -> Result<(), String> {
    ...
    // Parse and pair output paths with all input files
    type PathPair = (std::path::PathBuf, std::path::PathBuf);
    let mut file_paths = Vec::<PathPair>::with_capacity(user_args.paths.len());
    for input_path in &user_args.paths {
        let input_file = std::path::Path::new(&input_path)
            .canonicalize()
            .unwrap();

        let output_file = std::path::PathBuf::from(&user_args.out)
            .canonicalize()
            .unwrap()
            .join(std::path::PathBuf::from(input_file.file_name().unwrap()));

        file_paths.push((input_file, output_file));
    }

    // Create input encoding from user input
    let encoding = if user_args.encoding == "macintosh" {
        encoding_rs::MACINTOSH
    } else {
        encoding_rs::UTF_8
    };

    // Process loop for input files
    for (input_path, output_path) in &file_paths {
        println!("File: {}", input_path.to_str().unwrap());
        let edl_session = EDLParser::parse(input_path.to_str().expect("input file path must point to a valid EDL text file"), encoding)?;
        for track in &edl_session.tracks {
            println!("    Track name: {} - Events: {}", track.name, track.events.len());
